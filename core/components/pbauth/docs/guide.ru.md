...
**Требуется таблица.** Единственная возможность во всём компоненте, для которой
нужна миграция: связь «аккаунт у провайдера → пользователь» надо искать при
каждом входе, а искать её внутри JSON в профиле — перебор всей таблицы
пользователей. Пакет применяет миграцию сам; на сервере, куда файлы кладут
вручную, её надо применить руками (см. [справочник](#обновление-и-удаление)).
