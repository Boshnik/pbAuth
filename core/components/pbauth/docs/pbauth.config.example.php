<?php
/**
 * Настройки pbAuth для этого сайта.
 *
 * Установщик кладёт эту заготовку в core/App/config/pbauth.php, если её там ещё
 * нет. Дальше файл принадлежит сайту: pbAuth его только читает, обновление
 * компонента его не трогает.
 *
 * Раскомментируйте то, что нужно изменить. Всё остальное берётся из
 * core/components/pbauth/config/defaults.php — там же смотрите поставочные
 * значения. Пустая секция ничего не меняет.
 */
