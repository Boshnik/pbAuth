<form class="border rounded-4 p-5" action="{route 'resendVerification'}" method="post" pb-form>
    <input type="hidden" name="_token" value="{csrf_token}">
    <input type="hidden" name="honeypot" value="">

    <h3 class="text-center">{lang 'auth.form_resend_verification_title'}</h3>
    <p class="text-center">{lang 'auth.form_resend_verification_subtitle'}</p>
    ...
    <button type="submit" class="btn btn-dark w-100">
        ...
        <span role="status">{lang 'auth.form_resend_verification_submit'}</span>
    </button>
